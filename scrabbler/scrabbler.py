...
class Game:
    ...
    def __init__(self, filename="", board="scrabble"):
...
class Square:
    ...
    def tile(self, char):
        if self.tile and char != self.tile:
            raise IllegalMoveError("a tile already exists on this square")
        if char < 'A' or char > 'Z':
            raise IllegalMoveError("illegal move! Letter placed must be in the alphabet")
        self._tile = char
